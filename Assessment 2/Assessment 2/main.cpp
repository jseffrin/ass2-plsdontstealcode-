...
void DrawRectPrism(int x, int y, int z, int lengthx, int lengthy, int lengthz, int rot) {

	
	int xStart = (x - (lengthx/2));
	int yStart = (y + (lengthy/2));
	int zStart = (z + (lengthz/2));

	glColor3f(255, 206, 0);
	//front
	glBegin(GL_POLYGON);
	glVertex3f(xStart , yStart, zStart);
	glVertex3f(xStart + lengthx, yStart, zStart);
	glVertex3f(xStart + lengthx, yStart - lengthy, zStart);
	glVertex3f(xStart , yStart - lengthy, zStart);
	glEnd();
	//top
	glBegin(GL_POLYGON);
	glVertex3f(xStart, yStart, zStart);
	glVertex3f(xStart, yStart, zStart + lengthz);
	glVertex3f(xStart + lengthx, yStart, zStart + lengthz);
	glVertex3f(xStart + lengthx, yStart, zStart);
	glEnd();
	//left
	glBegin(GL_POLYGON);
	glVertex3f(xStart + lengthx, yStart, zStart);
	glVertex3f(xStart + lengthx, yStart, zStart + lengthz);
	glVertex3f(xStart + lengthx, yStart - lengthy, zStart + lengthz);
	glVertex3f(xStart + lengthx, yStart - lengthy, zStart);
	glEnd();
	//right
	glBegin(GL_POLYGON);
	glVertex3f(xStart, yStart, zStart);
	glVertex3f(xStart, yStart, zStart + lengthz);
	glVertex3f(xStart , yStart - lengthy, zStart + lengthz);
	glVertex3f(xStart, yStart - lengthy, zStart);
	glEnd();
	//bot
	glBegin(GL_POLYGON);
...
void drawSquare2() {
	...
	glBegin(GL_POLYGON);
	glVertex3f(-15, 70, 0.0);
	glVertex3f(15, 70, 0.0);
	glVertex3f(13, 75, 0.0);
	glVertex3f(11, 81, 0.0);
	glVertex3f(9, 85, 0.0);
	glVertex3f(7, 88, 0.0);
	glVertex3f(5, 90, 0.0);
	glVertex3f(4, 92, 0.0);
...
void display() {
	...
	//drawSquare();
	//drawSquare2;
	//DrawRectPrism(-100,100,0,10,100,1000,100);
	glutSwapBuffers();
};
